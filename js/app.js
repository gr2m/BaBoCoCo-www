window.app = {
  ...
  start: function() {
    ...
    app.router.on('route:home', function() {
      app.router.navigate('contacts/search', {
        trigger: true,
        replace: true
      });
    });

    app.router.on('route:showAllContacts', function(query) {
      ...
      app.currentView.on('search', function(search) {
        if (search) {
          app.router.navigate('contacts/search/'+search);
        } else {
          app.router.navigate('contacts/search');
        }
      });
    ...
    app.router.on('route:showFavorites', function() {
      if (app.currentView) app.currentView.remove();
    ...
    app.router.on('route:newContact', function() {
      if (app.currentView) app.currentView.remove();
      ...
      app.currentView.on('form:submitted', function(attrs) {
        app.contacts.create(attrs, {
          success: function(newContact) {
            app.router.navigate('contacts/' + newContact.id, true);
          }
        });
      });
        ...
        app.currentView.on('form:submitted', function(attrs) {
          contact.save(attrs, {
            success: function(contact) {
              app.router.navigate('contacts/' + contact.id, true);
            }
          });
        });
    ...
    app.router.on('route:showContact', function(id) {
      var contact = app.contacts.get(id);
    ...
    app.router.on('route:showSchedule', function() {
      if (app.currentView) app.currentView.remove();
    ...
    app.router.on('route:showAccount', function() {
      if (app.currentView) app.currentView.remove();
    ...
    app.router.on('route:logout', function() {
  ...
  bootstrap: function() {
    ...
    .forEach(app.contacts.create, app.contacts);
  }
};
